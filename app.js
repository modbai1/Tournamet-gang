...
function renderTournamentCard(t, enrollments) {
  ...
  const profitClass = hostProfit >= 0 ? 'profit-plus' : 'profit-minus';
  ...
  return `
    <article class="t-card ff-game-card">
      ...
      <p>Total Entry Collection: <strong>₹${totalCollection}</strong></p>
      <p class="${profitClass}">Tournament Profit (Host): <strong>₹${hostProfit}</strong></p>
