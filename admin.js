...
function render() {
  ...
  adminTournamentList.innerHTML = tournaments.length ? tournaments.map((t) => {
    const count = enrollments.filter((e) => e.tournamentId === t.id).length;
    const prizePool = Number(t.prizePool || 0);
    return `<div class="row"><span>${t.title} | ${t.date} | ₹${t.fee} | Prize ₹${prizePool} | ${t.mode} | ${t.map} | ${count}/${t.slots}</span><button data-delete-t="${t.id}" class="danger">Delete</button></div>`;
